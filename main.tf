terraform {
  required_version = ">= 1.0"

  required_providers {
    hcloud = {
      source = "registry.terraform.io/hetznercloud/hcloud"
    }
    hetznerdns = {
      source = "registry.terraform.io/timohirt/hetznerdns"
    }
  }

  backend "local" {
    path = ".tofu/tofu.tfstate"
  }
}

variable "hcloud_token" {
  sensitive = true
}
variable "hdns_token" {
  sensitive = true
}

provider "hcloud" {
  token = var.hcloud_token
}
provider "hetznerdns" {
  apitoken = var.hdns_token
}

variable "location" {
  default = "fsn1"
}

variable "workerCount" {
  # ALWAYS sync with workerCount in flake.nix
  default = 3
}

resource "hcloud_ssh_key" "roti_yubikey_5_nano" {
  name       = "roti_yubikey_5_nano"
  public_key = "ssh-rsa  cardno:23_118_572"
}

resource "hcloud_ssh_key" "roti_yubikey_5_nfc" {
  name       = "roti_yubikey_5_nfc"
  public_key = "ssh-rsa  cardno:20_560_926"
}

data "hetznerdns_zone" "rtti_de" {
  name = "rtti.de"
}

resource "hcloud_server" "dask-worker" {
  count       = var.workerCount 
  name        = "dask-worker-${count.index + 1}"
  server_type = "cx22"
  image       = "debian-12"
  location    = var.location
  ssh_keys    = [hcloud_ssh_key.roti_yubikey_5_nano.id, hcloud_ssh_key.roti_yubikey_5_nfc.id]

  public_net {
    ipv4_enabled = true
    ipv6_enabled = true
  }

  labels = {
    os-flavour = "nixos"
  }

  # TODO: get rid of the ip files, use just DNS
  provisioner "local-exec" {
    command = <<EOF
      echo ${self.ipv4_address} > public-ipv4-worker-${count.index + 1}
    EOF
  }
}

resource "hetznerdns_record" "dask-worker-A" {
  count   = var.workerCount 
  name    = "dask-worker-${count.index + 1}"
  zone_id = data.hetznerdns_zone.rtti_de.id
  value   = hcloud_server.dask-worker[count.index].ipv4_address
  type    = "A"
  ttl     = 60
}

resource "hcloud_server" "dask-scheduler" {
  name        = "dask-scheduler"
  server_type = "cx22"
  image       = "debian-12"
  location    = var.location
  ssh_keys    = [hcloud_ssh_key.roti_yubikey_5_nano.id, hcloud_ssh_key.roti_yubikey_5_nfc.id]

  public_net {
    ipv4_enabled = true
    ipv6_enabled = true
  }

  labels = {
    os-flavour = "nixos"
  }

  # TODO: get rid of the ip files, use just DNS
  provisioner "local-exec" {
    command = <<EOF
      echo ${self.ipv4_address} > public-ipv4-scheduler
    EOF
  }
}

resource "hetznerdns_record" "dask-schduler-A" {
  name    = "dask-scheduler"
  zone_id = data.hetznerdns_zone.rtti_de.id
  value   = hcloud_server.dask-scheduler.ipv4_address
  type    = "A"
  ttl     = 60
}
